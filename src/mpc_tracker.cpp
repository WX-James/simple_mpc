...
void mpc_tracker::initMPC(ros::NodeHandle& nh) {
  ...
  // 定时器回调：MPC主循环
  cmd_timer_ = nh_.createTimer(ros::Duration(0.10), &mpc_tracker::cmdCallback, this);
}
...
void mpc_tracker::cmdCallback(const ros::TimerEvent &e) {
  ...
  for(int i = 0; i < N_; i++)
  {
    desired_state(0, i) = ref_traj_msg.data[3 * (N_ - i) - 3];
    desired_state(1, i) = ref_traj_msg.data[3 * (N_ - i) - 2];
    desired_state(2, i) = ref_traj_msg.data[3 * (N_ - i) - 1];
    // 处理yaw角奇异值
    if(desired_state(2, i) - current_state[2] > M_PI) {
      desired_state(2, i) -= 2.0 * M_PI;
    } else if(desired_state(2, i) - current_state[2] < -M_PI) {
      desired_state(2, i) += 2.0 * M_PI;
    }
  }
